...
from modules.guide.pets_windows.builds import BuildPet
...
class PetInterface(QMainWindow):
    ...
    def set_widget(self):
        ...
        self.area_widgets.setStyleSheet('''
                                        QScrollBar:vertical {
    ...
    def adjust_information_display(self, path_img_status, layout_ref:QVBoxLayout, index):
        ...
        if index == 0:
            button_build_system.clicked.connect(lambda: self.open_build(2,'Luo Xiaomeng'))
            ...
            button_build_system.clicked.connect(lambda: self.open_build(0, 'Tigre Dourado do Ano Novo'))
            list_ref_ico = ['Imgs/natural_skill/tigre_dourado_do_ano_novo_01.jpg']
            buttons_natural_skills = [QPushButton() for i in range(list_ref_ico.__len__())]
            list_set_description = [lambda: self.description_skill('Sede de Sangue - PASSIVA',f'Cada dano que você causar até 10% desse \ndano pode retorna para restaurar seu HP.',label_name_skill,label_description_skill)]
            index = 0
            for i in buttons_natural_skills:
                i.clicked.connect(list_set_description[index])
                index += 1
            ...
            buttons_natural_skills = [QPushButton() for i in range(list_ref_ico.__len__())]
            list_set_description = [lambda: self.description_skill('Nether Basalt - ATIVAR',f'Causa dano igual a 25% do Ataque + 15% \ndo limite de HP em 4 alvos. Possui 50% de \nchance de causar tontura nos alvos. \nDura 1 Rodada',label_name_skill,label_description_skill),
